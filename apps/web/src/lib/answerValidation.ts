  ...
  switch (step) {
    case "heart-rate": {
      ...
      if (isNaN(answerNum)) {
        return {
          isCorrect: false,
          message: "Could you give me a number for the heart rate? (e.g., '75' or 'around 80 bpm')",
        }
      }
      ...
      if (Math.abs(answerNum - heartRate) <= tolerance) {
        return {
          isCorrect: true,
          message: `Spot on! The heart rate is approximately ${heartRate} bpm.`,
        }
      }
      ...
      if (answerNum < heartRate - tolerance) {
        return {
          isCorrect: false,
          message: "Actually, looks a bit faster than that. Try counting the boxes again!",
        }
      } else {
        return {
          isCorrect: false,
          message: "I'd say it's a bit slower than that. Look at the R-R interval once more.",
        }
      }
    }
