...
async function loadCourseContext(courseId: string) {
  ...
  if (!sessionWithRole) {
    redirect(`/login?next=/institution/courses/${courseId}/educators`)
  }
...
export default async function CourseEducatorsPage({ params, searchParams }: EducatorsPageProps) {
  const context = await loadCourseContext(params.courseId)
  const educators = await listMembers({ courseId: context.course.id, role: "EDUCATOR" })
  ...
  async function inviteEducatorAction(formData: FormData) {
    ...
    if (!pageContext.canManage) {
      throw new Error("Only institution admins can invite educators")
    }

    const email = String(formData.get("email") || "")
    const name = String(formData.get("educator_name") || "") || null
    ...
    const summary = await inviteMembers({
      ...
      role: "EDUCATOR",
      rows: [{ email, name, metadata: { educator_name: name } }]
    })

    revalidatePath(`/institution/courses/${courseId}/educators`)
    redirect(`/institution/courses/${courseId}/educators?result=${encodeSummary(summary)}`)
  }

  async function inviteEducatorCsvAction(formData: FormData) {
    "use server"
    ...
    if (!rawCsv.trim()) {
      ...
      redirect(`/institution/courses/${courseId}/educators?result=${encodeSummary(emptySummary)}`)
    }
    ...
    try {
      ...
      redirect(`/institution/courses/${courseId}/educators?result=${encodeSummary(parseSummary)}`)
    }

    const inviteRows = rows.map((row) => {
      const email = getField(row, ["educator_email", "email"])
      const educatorName = getField(row, ["educator_name", "name"])

      return {
        email,
        name: educatorName || null,
        metadata: {
  ...
  async function dropEducatorAction(formData: FormData) {
    "use server"
    ...
    if (!pageContext.canManage) {
      throw new Error("Only institution admins can remove educators")
    }
    ...
    revalidatePath(`/institution/courses/${courseId}/educators`)
    redirect(`/institution/courses/${courseId}/educators`)
  }

  return (
    <div className="space-y-4">
      <section className="surface-card p-6">
        <h2 className="text-xl font-semibold text-slate-900">Educators</h2>
        <p className="mt-1 text-sm text-slate-600">Invite and manage educators assigned to this course.</p>
      </section>
      ...
      {context.canManage ? (
        <section className="surface-card space-y-6 p-6">
          <div className="space-y-3">
            <h3 className="text-lg font-semibold">Invite Educators (Manual)</h3>
            <form action={inviteEducatorAction} className="grid gap-3 md:grid-cols-3">
              <input type="hidden" name="course_id" value={context.course.id} />
              <input name="email" type="email" required placeholder="educator_email" className="rounded-md border border-input bg-white px-3 py-2 text-sm" />
          ...
          <div className="space-y-3 border-t border-border pt-6">
            <div className="flex flex-wrap items-center justify-between gap-3">
              ...
              <a
                href={`/api/institution/courses/${context.course.id}/template?role=educator`}
                className="text-sm font-semibold text-primary hover:underline"
              >
                Download CSV Template
              </a>
            </div>
            <form action={inviteEducatorCsvAction} className="space-y-3">
              <input type="hidden" name="course_id" value={context.course.id} />
            ...
            <p className="text-xs text-slate-500">
              Supported columns: educator_email or email (required), educator_name (optional).
            </p>
          </div>
        </section>
      ) : (
          ...
          <table className="w-full text-left text-sm">
            ...
            <tbody>
              {educators.map((educator) => (
                <tr key={educator.membership_id} className="border-t border-border">
                  <td className="px-3 py-2">{educator.name || "—"}</td>
                  <td className="px-3 py-2">{educator.email}</td>
                  <td className="px-3 py-2">{educator.status}</td>
                  <td className="px-3 py-2">
                    {context.canManage ? (
                      <form action={dropEducatorAction}>
                        <input type="hidden" name="course_id" value={context.course.id} />
                        <input type="hidden" name="user_id" value={educator.user_id} />
